...
const contentData = {
  default: {
    titleKey: 'about.interactive.defaultTitle',
    descKey: 'about.interactive.defaultDesc',
    icon: "🚀"
  },
  laptop: {
    titleKey: 'about.interactive.laptopTitle',
    descKey: 'about.interactive.laptopDesc',
    icon: "💻"
  },
  monitor: {
    titleKey: 'about.interactive.monitorTitle',
    descKey: 'about.interactive.monitorDesc',
    icon: "🖥️"
  },
  coffee: {
    titleKey: 'about.interactive.coffeeTitle',
    descKey: 'about.interactive.coffeeDesc',
    icon: "☕"
  },
  trophy: {
    titleKey: 'about.interactive.trophyTitle',
    descKey: 'about.interactive.trophyDesc',
    icon: "🏆"
  }
};
...
export default function About() {
  ...
  const [hoveredKey, setHoveredKey] = useState('default');
  const [isFading, setIsFading] = useState(false);
  const sceneElementsRef = useRef({});
  const hoveredData = contentData[hoveredKey] || contentData.default;

  // Provide a state-based update UI instead of DOM manipulation
  const handleUpdateUI = (dataKey) => {
    const nextKey = contentData[dataKey] ? dataKey : 'default';
  ...
  useEffect(() => {
    ...
    // Load David's portrait and crop it to the monitor without stretching it.
    const textureLoader = new THREE.TextureLoader();
    const monitorAspect = 4.8 / 2.8;
    const photoTexture = textureLoader.load(profile.portrait, (texture) => {
      const imageAspect = texture.image.width / texture.image.height;

      if (imageAspect > monitorAspect) {
        const visibleWidth = monitorAspect / imageAspect;
        texture.repeat.set(visibleWidth, 1);
        texture.offset.set((1 - visibleWidth) / 2, 0);
      } else {
        const visibleHeight = imageAspect / monitorAspect;
        const faceFocusY = 0.3;
        const cropTop = Math.max(
          0,
          Math.min(1 - visibleHeight, faceFocusY - visibleHeight / 2)
        );
        texture.repeat.set(1, visibleHeight);
        // Texture V coordinates run bottom-to-top. This converts the desired
        // top-based face crop so the full head stays visible on the monitor.
        texture.offset.set(0, 1 - cropTop - visibleHeight);
      }
  ...
  return (
    <AboutSection id="about" $isRTL={isRTL}>
      ...
      <UILayer>
        <HeaderBox>
          <Badge>{t('about.interactiveBadge')}</Badge>

          <InfoContent className={isFading ? 'fade-out' : ''}>
            <TitleRow>
              <CardIcon>{hoveredData.icon}</CardIcon>
              <CardTitle>{t(hoveredData.titleKey)}</CardTitle>
            </TitleRow>
            <CardDesc>
              {t(hoveredData.descKey)}
            </CardDesc>
          </InfoContent>
        </HeaderBox>
      </UILayer>

...
const Testimonials = () => {
  ...
  useEffect(() => {
    ...
    // Use the current DB monogram as the source for the particle field.
    const logoWidth = Math.min(window.innerWidth * 0.82, 900);
    const logoAspect = 214 / 118;
    const offW = Math.ceil(logoWidth) + 100;
    const offH = Math.ceil(logoWidth / logoAspect) + 100;
    ...
    const sampleLogoParticles = (imgData) => {
      particles.length = 0;
      ...
      for (let y = 0; y < offH; y += STEP) {
        for (let x = 0; x < offW; x += STEP) {
          ...
          if (imgData.data[idx + 3] > 128 && brightness > 90) {
    ...
    brandLogo.onload = () => {
      ...
      // Detect the light monogram inside the dark image so its built-in
      // padding does not make the particle logo look too small.
      octx.drawImage(brandLogo, 0, 0);
      const sourcePixels = octx.getImageData(0, 0, brandLogo.width, brandLogo.height);
      let minX = brandLogo.width;
      let minY = brandLogo.height;
      let maxX = 0;
      let maxY = 0;

      for (let y = 0; y < brandLogo.height; y += 1) {
        for (let x = 0; x < brandLogo.width; x += 1) {
          const idx = (y * brandLogo.width + x) * 4;
          const brightness = (
            sourcePixels.data[idx] +
            sourcePixels.data[idx + 1] +
            sourcePixels.data[idx + 2]
          ) / 3;

          if (sourcePixels.data[idx + 3] > 128 && brightness > 90) {
      ...
      const fitScale = Math.min(
        (offW - 80) / sourceWidth,
        (offH - 60) / sourceHeight
      );
      const drawWidth = sourceWidth * fitScale;
      const drawHeight = sourceHeight * fitScale;
      const drawX = (offW - drawWidth) / 2;
      const drawY = (offH - drawHeight) / 2;
      ...
      sampleLogoParticles(octx.getImageData(0, 0, offW, offH));
    };
    brandLogo.src = '/brand/db-monogram.png';
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {isActive ? (
                    <div style={{ marginLeft: isRTL ? '0' : '20px', marginRight: isRTL ? '20px' : '0', display: 'flex', flexDirection: 'column', height: '100%' }}>
